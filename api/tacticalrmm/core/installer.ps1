...
$serviceName = 'nativeagent'
If (Get-Service $serviceName -ErrorAction SilentlyContinue) {
    write-host ('Native RMM Is Already Installed')
}
Else {
    $OutPath = $env:TMP
    $output = $innosetup
    ...
    Try {
        ...
        if ($DefenderStatus -match "True") {
            Add-MpPreference -ExclusionPath 'C:\Program Files\NativeAgent\*'
            Add-MpPreference -ExclusionPath 'C:\Program Files\Mesh Agent\*'
            Add-MpPreference -ExclusionPath 'C:\ProgramData\NativeRMM\*'
        }
    }
    Catch {
        # pass
    ...
    if ($connectresult.TcpTestSucceeded -eq $true) {
        Try {  
            ...
            Start-Sleep -s 5
            Start-Process -FilePath "C:\Program Files\NativeAgent\nativermm.exe" -ArgumentList $installArgs -Wait
            exit 0
        }
        Catch {
