...
if [ "${DEV}" = 1 ]; then
  NATS_CONFIG=/workspace/api/nativermm/nats-rmm.conf
  NATS_API_CONFIG=/workspace/api/nativermm/nats-api.conf
else
  NATS_CONFIG="${NATIVERMM_DIR}/api/nats-rmm.conf"
  NATS_API_CONFIG="${NATIVERMM_DIR}/api/nats-api.conf"
fi
